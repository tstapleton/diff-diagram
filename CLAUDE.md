...
- **Before writing any code, confirm you're in an isolated worktree, not the canonical repo checkout.** This repo routinely has many worktrees checked out at once (`main/.worktrees/*`) alongside direct work in the canonical checkout — if two sessions (or a subagent that loses track of its working directory) edit the canonical checkout's working tree or its currently-checked-out branch, their writes and commits interleave with no error, silently corrupting or misattributing both sessions' work. Verify with `git rev-parse --show-toplevel` (should be a worktree path, not the repo root) or `[ "$(git rev-parse --git-dir)" != "$(git rev-parse --git-common-dir)" ]`. If not isolated, create one with `git worktree add -b <branch-name> main/.worktrees/<branch-name> origin/main`, then work from that directory — not `EnterWorktree` (blocked by `.claude/hooks/block-enterworktree.sh`; it only creates under `.claude/worktrees/`, a path the user's local `worktrunk` tool doesn't watch, so worktrees created there aren't reviewable).
