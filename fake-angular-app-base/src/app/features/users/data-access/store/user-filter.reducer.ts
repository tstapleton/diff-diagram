import { defaultFilter, type FilterModel } from "../../user-list/filter.model";
import type { UserFilterAction } from "./user-filter.actions";
...
export function userFilterReducer(
	...
	switch (action.type) {
		...
		case "SET_PAGE":
			return {
				...state,
				filter: { ...state.filter, page: action.payload as number },
			};
		default:
			return state;
	}
