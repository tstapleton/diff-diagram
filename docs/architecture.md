...
- `'focused'` — applies collapse rules:
  ...
  7. Group out-of-scope nodes by directory, capped at `OOS_GROUP_DEPTH` (3) segments below `sourceRoot` rather than each file's exact immediate parent (`oosGroupDir`) — every group always collapses to one `type: 'directory'` node (`makeDirNode`), the same treatment `'collapsed'` mode gives out-of-scope directories (rules 3–5 above, in-scope's open/partial/closed split, don't apply to out-of-scope groups). The node's `diff`/`magnitude` aggregate from its members, and a changed edge into/out of the group still survives rules 8–9 below, so the box still renders at full opacity when something inside it changed — it just no longer names which specific member did
  8. Remap edges: if a node was collapsed, redirect its edges to the stub/directory node's ID; an edge touching a dropped partial member (no stand-in node, no collapsedMap entry) is dropped instead
  9. Deduplicate edges (multiple original edges may map to the same collapsed→collapsed edge), keeping the highest-priority diff state on collision
  10. Drop self-loops (both endpoints collapsed to the same node)

In-scope stub nodes have `type: 'stub'`, `diff: 'unchanged'` (a stub only ever forms when nothing inside it changed at either the file or edge level, so hardcoding `'unchanged'` is accurate, not a placeholder). Both are a rendering abstraction — they represent a directory, not a real file. Out-of-scope groups use `type: 'directory'` instead, identical to `'collapsed'` mode's out-of-scope boxes (see below).
...
Node dimensions: regular nodes 140×40px, in-scope stub nodes 120×32px. An out-of-scope directory box (Focused or Collapsed view — both use the same `type: 'directory'` node) uses the regular 140×40px sizing instead of the compact stub size, since it needs room for a path subtitle the way an out-of-scope file does — see `nodeDims` in `layout.ts`.
